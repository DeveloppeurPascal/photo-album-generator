...
function Get_Photo_Larg: Integer;
begin
  if (f.Image1.Picture.Width = f.Image1.Picture.Height) then
    result := f.Image1.Picture.Width
  else if (f.Image1.Picture.Width > f.Image1.Picture.Height) then
    if (f.Image1.Picture.Width > 600) then
      result := 600
    else
      result := f.Image1.Picture.Width
      { endif }
  else
    result := (f.Image1.Picture.Width * Get_Photo_Haut)
      div f.Image1.Picture.Height
    { endif }
end;

function Get_Photo_Haut: Integer;
begin
  if (f.Image1.Picture.Height = f.Image1.Picture.Width) then
    result := f.Image1.Picture.Height
  else if (f.Image1.Picture.Height > f.Image1.Picture.Width) then
    if (f.Image1.Picture.Height > 600) then
      result := 600
    else
      result := f.Image1.Picture.Height
      { endif }
  else
    result := (f.Image1.Picture.Height * Get_Photo_Larg)
      div f.Image1.Picture.Width
    { endif }
end;
