...
uses
  ...
  ActnList, StdActns, Menus, ExtDlgs;
...
uses fileCtrl, u_param, f_operation_en_cours, f_apropos, f_album_info, f_photo_ajout,
...
begin
  ...
  if (Album_Ouvert)
  then
    if (mrYes = MessageDlg ('Un album est actuellement ouvert, voulez-vous le fermer ?', mtConfirmation, [mbYes, mbNo], 0))
    then
      Fermer1Click (Sender)
    else
      exit;
    {endif}
  {endif}
  if (NouvelAlbum.InitialDir = '')
  then
    NouvelAlbum.InitialDir := prog_folder+'album';
  {endif}
  if (NouvelAlbum.Execute)
  then
    if (DirectoryExists (NouvelAlbum.FileName))
    then
      MessageDlg ('Le dossier '+ExtractFileName (NouvelAlbum.FileName)+' existe déjà. Création de l''album impossible', mtError, [mbCancel], 0)
    else
      begin
        oec_Ouverture (20);
        ...
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'ressources\autorun.inf'), PChar (album_folder+'\autorun.inf'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'ressources\shelexec.exe'), PChar (album_folder+'\shelexec.exe'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'ressources\pag.ico'), PChar (album_folder+'\pag.ico'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'ressources\barre.gif'), PChar (album_folder+'\btn\barre.gif'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'ressources\haut.gif'), PChar (album_folder+'\btn\haut.gif'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'ressources\gauche.gif'), PChar (album_folder+'\btn\gauche.gif'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'ressources\droite.gif'), PChar (album_folder+'\btn\droite.gif'), False);
        oec_Operation_Suivante;
        ForceDirectories (album_folder+'\mdl');
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'mdl\browser.mdl'), PChar (album_folder+'\mdl\browser.mdl'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'mdl\credit.mdl'), PChar (album_folder+'\mdl\credit.mdl'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'mdl\index.mdl'), PChar (album_folder+'\mdl\index.mdl'), False);
        oec_Operation_Suivante;
        CopyFile (PChar (Prog_Folder+'mdl\photo.mdl'), PChar (album_folder+'\mdl\photo.mdl'), False);
        oec_Operation_Suivante;
        Param_Ouvrir_Album (album_folder+'\'+ExtractFileName (album_folder)+'.pag');
        oec_Operation_Suivante;
...
begin
  if (Album_Modifie)
  then
    if (mrYes = MessageDlg ('Cet album a été modifié mais pas généré, voulez-vous le générer avant de le fermer ?', mtConfirmation, [mbYes, mbNo], 0))
    then
      N3Gnrerlalbum1Click (Sender);
    {endif}
  {endif}
...
begin
  ...
  try
    if (OuvrirJPG.InitialDir = '')
    then
      OuvrirJPG.InitialDIR := Param_Album.ReadString ('photos', 'default_img_path', Album_Path);
    {endif}
    if (OuvrirJPG.Execute)
    then
      for i := 0 to Pred (OuvrirJPG.Files.Count) do
        begin
          if (i = 0)
          then
            Param_Album.WriteString ('photos', 'default_img_path', ExtractFilePath (OuvrirJPG.Files [i]));
          {endif}
...
begin
  ...
  for i := 1 to Param_Album.ReadInteger ('photos', 'nb_photos', 0) do
    begin
      Generer_Page (Album_Path+'mdl\photo.mdl', Album_Path+'page'+IntToStr (i)+'.html', i);
      oec_Operation_Suivante;
    end;
  {endfor}
  oec_Fermeture;
...
begin
  ...
  then
    ...
    {endif}
  {endif}
  if (OuvrirPAG.InitialDir = '')
  then
    OuvrirPAG.InitialDir := prog_folder+'album';
  {endif}
  if (OuvrirPAG.Execute)
  then
    begin
...
begin
  WinExec (PChar (ExtractFilePath (Application.ExeName)+'ressources\shelexec '+album_path+'index.html'), sw_normal);
end;
