...
begin
  ...
  then
    begin
      ...
      then
        res := 'Page generated by PhotoAlbumGenerator from Patrick "pprem" Prémartin on the '+DateToStr (Now);
      {endif}
...
begin
  ...
  then
    begin
      result := True;
      delete (ch, n, length ('<HEAD>')); // attention, le blanc derrière HEAD dans le résultat permet d'éviter de boucler indéfiniement !
      ch := copy (ch, 1, pred (n))+
            '<head >'#10#13+
  ...
  then
    begin
      result := True;
      delete (ch, n, length ('</BODY>')); // attention, le blanc derrière /BODY dans le résultat permet d'éviter de boucler indéfiniement !
      ch := copy (ch, 1, pred (n))+
            '<small><small><address>Page generated by <a href="http://www.olfsoft.com" target="_top">PhotoAlbumGenerator</a> from <a href="mailto:pprem@pprem.net">Patrick "pprem" Prémartin</a> on the '+DateToStr (Now)+'.</address></small></small>'#10#13+
            '</body >'#10#13+
            copy (ch, n, succ (length (ch)-n));
    end;
  {endif}
...
begin
  ...
  try
    ...
    then // si le fichier de modèle n'existe pas, on signale une erreur
      raise Exception.Create ('Fichier modèle '+modele+' inexistant !');
    {endif}
    mdl := TStringList.Create; // contiendra le modèle à appliquer
    lst_if := TStringList.Create; // contiendra les éventuels conditionnements des blocs du programme
    lst_liste := TStringList.Create; // stockage des indices en cas de listes imbriquées
    lst_photo := TStringList.Create; // stockage des indices en cas de listes imbriquées
    try
      ...
      while (i < mdl.Count) do // on parcourt toutes les lignes du modèle
        begin
          ...
          then // une condition est détectée, on va la vérifier
            begin
              reboucler := true;
// gérer ici les éventuels conditionnements de parties du modèle des pages
          ...
          {endif}
          if ((not reboucler) and (pos ('!$/IF$!', upcase_ch) > 0)) // on dépile une condition
          then
            begin
              reboucler := true;
...
// liste des photographies
          ...
          then
            begin
              ...
              then
                lst_liste.Add (IntToStr (No_Debut_Boucle)); // stockage de la valeur précédente
              {endif}
          ...
          then
            begin
              ...
              then
                ...
                i := No_Debut_Boucle -1; // le -1 est là car on réincrémente I après !
              {endif}
            end;
          {endif}
...
// prise en compte des marqueurs
          ...
          then
            begin
              while modification_marqueur (ch, num_photo) do; // remplacement des mots-clés reconnus par leur valeur
